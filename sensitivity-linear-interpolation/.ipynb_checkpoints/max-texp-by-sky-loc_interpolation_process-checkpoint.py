...
Description     : This script processes gravitational wave (GW) localization maps to identify events
                  ...
                  This script is adapted from https://github.com/criswellalexander/uvex-followup.
...
from sensitivity.background import galactic, zodiacal
from sensitivity import ultrasatmetrics
...
scaling_file_defaults = os.path.join(localpath, "sensitivity/scaling_default.txt")
with open(scaling_file_defaults, "r") as json_file:
    SCALING = json.load(json_file)

# Paths to Zodiacal Light Spectrum files
ZODI_SPATIAL = os.path.join(
    localpath, "sensitivity/background/data", "Leinert97_table17.txt"
)
ZODI_SPEC = os.path.join(
    localpath, "sensitivity/background/data", "scaled_zodiacal_spec.txt"
)
...
def get_pixel_texp(ipix, nside, obstime, source_rate, band, bandpass, area) -> list:
    """
    Calculate the required exposure time for a specific HEALPix pixel to reach SNR of 5.

    Args:
        ipix (int): HEALPix pixel index.
        nside (int): HEALPix Nside parameter.
        obstime (Time): Observation time.
        source_rate (float): Estimated observed source count rate.
        band (str): Observation band ('nuv' or 'fuv').
        bandpass: Bandpass filter used for observations.
    ...
    try:
        ...
        zodi_obs = Observation(zodi_spec, bandpass, force="extrap")
        ...
        if band.lower() == "nuv":
            ...
            galactic_obs = Observation(galactic_spec, bandpass, force="extrap")

            sky_background = zodi_obs.countrate(area=area) + galactic_obs.countrate(
                area=area
            )
...
def get_max_texp(
    credible_levels, nside, start_time, m_obs, band, area, verbose=True
) -> float:
    """
    ...
    try:
        ...
        if band.lower() == "nuv":
            bandpass = ultrasat.detector.bandpasses["NUV"]
        else:
            raise ValueError(f"Unsupported band: {band}")

        # Calculate source count rate

        observation = Observation(source_spectrum, bandpass, force="extrap")
        ...
        for ipix, cl in enumerate(credible_levels):
            if cl <= 0.9:
                print("ipix :", ipix)
                pix_texp = get_pixel_texp(
                    ipix, nside, start_time, source_rate, band, bandpass, area
                )
                if pix_texp is None:
                    count += 1
                    continue
        ...
        for progress, file_id in enumerate(ids, start=1):
            ...
            try:
                texp_max = get_max_texp(
                    credible_levels, nside, start_time, m_obs, band, area, verbose=False
                )
                if texp_max is None:
                    logging.info(

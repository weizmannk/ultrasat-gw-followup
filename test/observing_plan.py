# python observing_plan.py ./data/5578.ecsv  5578_MOVIE.gif --dpi 300 --still   5578.pdf
...
def animate(
    ...
    with status("making animation"), quantity_support():
        ...
        with status("adding observation footprints"):
            ...
            table["area"] = np.empty((len(table), visits)) * hpx.pixel_area.to(
                u.deg**2
            )
